...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        curr_range = 0
        while curr_range < self.lower_bound and not place.is_hive:
            place = place.entrance
            curr_range += 1
        while curr_range <= self.upper_bound and not place.is_hive:
            if len(place.bees) > 0:
                break
            place = place.entrance
            curr_range += 1
        if place.is_hive or curr_range > self.upper_bound:
            return None
        return random_else_none(place.bees)  # REPLACE THIS LINE
        # END Problem 3 and 4
...
def random_else_none(s):
    """Return a random element of sequence S, or return None if S is empty."""
    assert isinstance(s, list), "random_else_none's argument should be a list but was a %s" % type(s).__name__
    if s:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        print("DEBUG: remaining bees health", [bee.health for bee in self.place.bees])
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            target = random_else_none(self.place.bees)
            if target is not None:
                self.eat_bee(target)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if cls.queen_deployed:
            return None
        else:
            cls.queen_deployed = True
    ...
    def action(self, gamestate):
        ...
        while curr_place is not None:
            if curr_place.ant and not curr_place.ant.is_doubled:
                curr_place.ant.double()
            if isinstance(curr_place.ant, ContainerAnt) \
                    and curr_place.ant.ant_contained \
                    and not curr_place.ant.ant_contained.is_doubled:
