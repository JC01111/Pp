...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        define_lambda = do_lambda_form(Pair(formals, body), env)
        env.bindings[symbol] = define_lambda
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.rest != nil:
        if is_scheme_false(scheme_eval(expressions.first, env)):
            return scheme_eval(expressions.first, env, False)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            if clause.rest is nil: #clause.rest to find if the expression doesn't have value
                return test
            return eval_all(clause.rest, env) #evaluate the rest value of last expression
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while bindings != nil:
        sub_bindings = bindings.first
        validate_form(sub_bindings, 2, 2)
        names = Pair(sub_bindings.first, names)
        vals = Pair(scheme_eval(sub_bindings.rest.first, env), vals)
