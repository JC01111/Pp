    ...
    if len(typed_words) == len(source_words) == 0:
        ...
        for i in range(min(len(typed_words),len(source_words))):
            if typed_words[i] == source_words[i]:
                num_correct_words += 1
        return num_correct_words / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    curr_diff = diff_function(typed_word, word_list[0], limit)
    words = word_list[0]
    if typed_word in word_list:
        return typed_word
    for index in range(1, len(word_list)):
        new_diff = diff_function(typed_word, word_list[index], limit)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for index in range(len(typed)):
        if typed[index] == prompt[index]:
            ...
            break
    cal_progress = correct_num / len(prompt)
    upload({'id': user_id, 'progress': cal_progress})
    return cal_progress
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    new_time = [[times_per_player[i][j + 1] - times_per_player[i][j] for j in range(len(times_per_player[i]) - 1)] for i in range(len(times_per_player))]
    return match(words, new_time)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    list = [[] for index in player_indices]
    for b in range(len(get_all_times(match)[0])):
        count = 0
        smallest = min([get_all_times(match)[a][b] for a in player_indices])
        for c in player_indices:
            if smallest == get_all_times(match)[c][b]:
                count += 1
                if count == 1:
                    list[c].append(get_word(match, b))
    return list
    # END PROBLEM 10
